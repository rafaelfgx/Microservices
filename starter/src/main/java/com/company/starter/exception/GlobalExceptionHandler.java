...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handle(final HandlerMethodValidationException exception) {
        final var errors = exception
            .getParameterValidationResults()
            .stream()
            .flatMap(result -> result.getResolvableErrors().stream().map(error -> "%s: %s".formatted(result.getMethodParameter().getParameterName(), error.getDefaultMessage())))
            .sorted()
            .toList();
    ...
    public ResponseEntity<Void> handle(final Exception exception) {
        final var status = exception instanceof ErrorResponse errorResponse ? errorResponse.getStatusCode() : HttpStatus.INTERNAL_SERVER_ERROR;
