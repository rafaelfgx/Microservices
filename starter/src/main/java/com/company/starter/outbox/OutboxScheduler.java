...
public class OutboxScheduler {
    ...
    public void handle() {
        ...
        try {
            ...
            final var sent = futures.stream().map(CompletableFuture::join).flatMap(Optional::stream).collect(Collectors.toSet());
    ...
    private CompletableFuture<Optional<UUID>> each(final Outbox outbox) {
        try {
            ...
            return kafkaTemplate.send(outbox.getTopic(), outbox.getKey(), outbox.getData()).orTimeout(properties.lock().toMillis(), TimeUnit.MILLISECONDS).handle((_, throwable) -> handle(outbox, throwable));
        } catch (final Exception exception) {
