...
class OutboxServiceTest {
    ...
    void shouldFailWhenDataIsNull() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> transactionTemplate.executeWithoutResult(_ -> outboxService.save(TOPIC, KEY, null)));
    ...
    @NullAndEmptySource
    @ValueSource(strings = " ")
    void shouldFailWhenTopicIsNullOrBlank(final String topic) {
        Assertions.assertThrows(IllegalArgumentException.class, () -> transactionTemplate.executeWithoutResult(_ -> outboxService.save(topic, KEY, "value")));
        Assertions.assertTrue(mongoTemplate.findAll(Outbox.class).isEmpty());
    }
    ...
    @NullAndEmptySource
    @ValueSource(strings = " ")
    void shouldFailWhenKeyIsNullOrBlank(final String key) {
        Assertions.assertThrows(IllegalArgumentException.class, () -> transactionTemplate.executeWithoutResult(_ -> outboxService.save(TOPIC, key, "value")));
        Assertions.assertTrue(mongoTemplate.findAll(Outbox.class).isEmpty());
    }
