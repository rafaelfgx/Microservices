...
class GlobalExceptionHandlerTest {
    ...
    @Test
    void shouldHandleWithProblemDetailWhenResponseStatusExceptionOccurs() {
        ...
        Assertions.assertEquals("Message", response.getBody().getDetail());
    }
    ...
    @Test
    void shouldHandleAsMethodNotAllowedWhenHttpRequestMethodNotSupportedExceptionOccurs() {
        assertions(globalExceptionHandler.handle(new HttpRequestMethodNotSupportedException("PATCH")), HttpStatus.METHOD_NOT_ALLOWED);
    }

    @Test
    void shouldHandleAsUnsupportedMediaTypeWhenHttpMediaTypeNotSupportedExceptionOccurs() {
        assertions(globalExceptionHandler.handle(new HttpMediaTypeNotSupportedException("Message")), HttpStatus.UNSUPPORTED_MEDIA_TYPE);
    }

    @Test
    void shouldHandleAsBadRequestWhenMissingServletRequestParameterExceptionOccurs() {
        assertions(globalExceptionHandler.handle(new MissingServletRequestParameterException("name", "String")), HttpStatus.BAD_REQUEST);
    }

    @Test
    void shouldHandleAsBadRequestWhenHandlerMethodValidationExceptionOccurs() {
        final var parameter = Mockito.mock(MethodParameter.class);
        Mockito.when(parameter.getParameterName()).thenReturn("age");
        final var result = Mockito.mock(ParameterValidationResult.class);
        Mockito.when(result.getMethodParameter()).thenReturn(parameter);
        Mockito.when(result.getResolvableErrors()).thenReturn(List.of(new DefaultMessageSourceResolvable(new String[]{"Positive"}, "must be positive")));
        final var validationResult = Mockito.mock(MethodValidationResult.class);
        Mockito.when(validationResult.getParameterValidationResults()).thenReturn(List.of(result));
        assertions(globalExceptionHandler.handle(new HandlerMethodValidationException(validationResult)), List.of("age: must be positive"));
    }
